...
- [x] Le script ajoute une entrée dans `data/cities.json` avec les URLs Hugging Face
- [x] Le script effectue un `git commit` et `git push` vers le repo (uniquement `git commit` en mode dev puis on active `git push` une fois le script fonctionnelle)
- [x] Cela déclenche un déploiement Vercel (via Webhook implicite)
...
- [x] Code complete
- [x] Tests written and passing (mock git push)
- [x] Code reviewed
- [x] Documentation updated
- [x] Deployed to dev/prod environment
