...
def apply_custom_patches(maptoposter_dir: Path):
    ...
    overrides_dir = worker_dir / "custom_overrides"
    ...
    for fmt_name, fmt_dims in FORMATS.items():
        ...
        for t in themes_to_run:
            ...
            cmd = [
                 ...
                 "--latitude", str(lat),
                 "--longitude", str(lon),
                 # The --distance argument in create_map_poster.py is now treated as the full diameter (download radius).
                 # We want the map to cover the full diameter of the city (2 * radius).
                 "--distance", str(int(dist * 2000)), # Convert KM radius to 'Diameter' in Meters
                 "--width", str(width),
                 "--height", str(height),
