...
def fetch_graph(point, dist) -> MultiDiGraph | None:
    ...
    # Optimization: For large areas, 'all' network type is too heavy (especially for RPi).
    # Adapt network_type based on distance.
    # If radius > 8km, switch to 'drive' (roads only), skipping paths/pedestrians which clutter large maps.
    if dist > 10000:
        print(f"ℹ️  Large area detected ({dist:.0f}m). Using 'drive' network type for performance.")
        net_type = 'drive'
    else:
        net_type = 'all'

    # Include net_type in cache key to avoid collisions
    graph = f"graph_{lat}_{lon}_{dist}_{net_type}"
    ...
    try:
        g = ox.graph_from_point(point, dist=dist, dist_type='bbox', network_type=net_type, truncate_by_edge=True)
...
def create_poster(
    ...
    with tqdm(
        ...
        # Optimized: Download a fixed large area (dist) to enable caching across formats
        download_dist = dist 
        # Actual display distance based on current aspect ratio
        compensated_dist = (dist / 2) * (max(height, width) / min(height, width))
        g = fetch_graph(point, download_dist)
        if g is None:
